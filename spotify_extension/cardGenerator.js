...
if (typeof self.CARD_GENERATOR_LOADED === 'undefined') {
  self.CARD_GENERATOR_LOADED = true;
   ...
   * Uses unbiased conversion to avoid modulo bias
   */
  function secureRandomInt(max) {
    if (
      typeof self !== 'undefined' && self.crypto && self.crypto.getRandomValues ||
  ...
  function randomDigits(length) {
    let out = '';
    for (let i = 0; i < length; i++) out += randomDigit();
    return out;
  }
  ...
  /**
   * Parse BIN pattern — supports digits and 'x'/'X' as random digit wildcards.
   * e.g. "4610xxxx" fills x with random digits.
   * @param {string} bin - BIN pattern
   * @param {number} maxLen - max digits to extract
   * @returns {string} digit string
   */
  function parseBinPattern(bin, maxLen) {
    let out = '';
    for (const ch of String(bin)) {
      if (out.length >= maxLen) break;
      const lc = ch.toLowerCase();
      if (lc === 'x') out += randomDigit();
      else if (/\d/.test(lc)) out += lc;
    }
    return out;
  }
  ...
  function calculateLuhnCheckDigit(partialNumber) {
    ...
    for (let i = 0; i < digits.length; i++) {
      ...
      // Double every 2nd digit from the right (1, 3, 5...)
      // Since we are adding ONLY the check digit at the end, the current rightmost
      // digit will be at position 1 (even index from 0-based right) and should be doubled.
      if (i % 2 === 0) {
        digit *= 2;
        if (digit > 9) digit -= 9;
      }
  ...
  function validateLuhn(cardNumber) {
    ...
    for (let i = 0; i < digits.length; i++) {
      const position = digits.length - 1 - i;
      let digit = digits[position];
      // Check digit (pos 0) NOT doubled. Double pos 1, 3, 5...
      if (i % 2 === 1) {
        digit *= 2;
        if (digit > 9) digit -= 9;
      }
  ...
  function verifyLuhnChecksum(digits) {
    if (typeof digits === 'string') return validateLuhn(digits);
    return validateLuhn(digits.join(''));
  }
   ...
   * Supports x-wildcard BIN patterns (e.g. "4610xxxx").
   * @param {string} bin - BIN (2–12 chars, digits + optional 'x')
   ...
   * @returns {string} Full card number (passes double-verified Luhn)
   */
  function generateCardFromBIN(bin, length = null, cardTypeOverride = '') {
    // Expand x-wildcards in BIN to detect type cleanly
    const binFixed = sanitizeDigits(bin.toLowerCase().replace(/x/g, '0'));
    if (!binFixed || binFixed.length < 2 || binFixed.length > 12) {
      throw new Error('BIN must be 2–12 digits (supports x wildcard)');
    }

    const cardType = detectCardType(binFixed) ||
    ...
    const cardLength = resolveCardLength(binFixed, length, cardType);

    if (binFixed.length >= cardLength) {
      throw new Error(`BIN (${binFixed.length} digits) too long for card length ${cardLength}`);
    }

    // Max retries for double-verify
    for (let attempt = 0; attempt < 5; attempt++) {
      // Parse BIN pattern with wildcard support (fills 'x' randomly)
      const binParsed = parseBinPattern(bin, cardLength - 1);
      // Fill remaining middle digits
      const remainLen = cardLength - 1 - binParsed.length;
      const withoutCheck = binParsed + randomDigits(remainLen);
      const checkDigit = calculateLuhnCheckDigit(withoutCheck);
      const full = withoutCheck + checkDigit;

      // Double-verify
      if (validateLuhn(full)) return full;
    }

    // Fallback: straightforward generation (guaranteed)
    const middleLen = cardLength - binFixed.length - 1;
    const middle = randomDigits(middleLen);
    const withoutCheck = `${binFixed}${middle}`;
    return `${withoutCheck}${calculateLuhnCheckDigit(withoutCheck)}`;
  }
